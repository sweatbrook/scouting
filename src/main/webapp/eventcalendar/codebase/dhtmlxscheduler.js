...
function getOffsetRect(e) {
    var t = e.getBoundingClientRect(), s = document.body, i = document.documentElement, a = window.pageYOffset || i.scrollTop || s.scrollTop, n = window.pageXOffset || i.scrollLeft || s.scrollLeft, r = i.clientTop || s.clientTop || 0, d = i.clientLeft || s.clientLeft || 0, o = t.top + a - r, l = t.left + n - d;
...
window.dhtmlXScheduler = window.scheduler = {version: "4.3.0"}, window.dhtmlx || (dhtmlx = function (e) {
    ...
    var s = e.replace(/[\/]+/gi, "/").split("/"), i = null, a = 1;
    if (!s.length)
        return[];
    if ("." == s[0])
        i = [t];
    ...
    for (a; a < s.length; a++)
        i = this._getAllNamedChilds(i, s[a]);
    ...
    for (var i = 0; i < e.length; i++)
        for (var a = 0; a < e[i].childNodes.length; a++)
            _isKHTML ? e[i].childNodes[a].tagName && e[i].childNodes[a].tagName.toUpperCase() == t && (s[s.length] = e[i].childNodes[a]) : e[i].childNodes[a].tagName == t && (s[s.length] = e[i].childNodes[a]);
    return s
    ...
    function h(e, t, s) {
        return"object" != typeof e && ("function" == typeof t && (s = t, t = ""), e = {text: e, type: t, callback: s}), e
    }
    function _(e, t, s, i) {
        return"object" != typeof e && (e = {text: e, type: t, expire: s, id: i}), e.id = e.id || u.uid(), e.expire = e.expire || u.expire, e
    }
    var c = null;
    ...
    u.seed = (new Date).valueOf(), u.uid = function () {
        ...
        if (a && (i = this.styles[a], t = !0), this.callEvent("onRowMark", [e, t, s, a]) && (i = this.styles[t ? s : "clear"] + i, this.obj[this._methods[0]](e, i), a && a.details)) {
            ...
            for (var n = 0; n < a.details.length; n++)
                a.details[n] && this.obj[this._methods[1]](e, n, i)
        }
    }, getState: function (e) {
        ...
        s && (t = {value: t, details: s, toString: function () {
        ...
        var s = [], i = [];
        for (var a in e)
            e.hasOwnProperty(a) && (s.push(this.serialize_one(e[a], a + this.post_delim)), i.push(a));
        ...
        if (e) {
            ...
            }, this, !0), i = this.serverProcessor + (this._user ? getUrlSymbol(this.serverProcessor) + ["dhx_user=" + this._user, "dhx_version=" + this.obj.getUserData(0, "version")].join("&") : "");
            if ("GET" == this._tMode)
                s.loadXML(i + (-1 != i.indexOf("?") ? "&" : "?") + this.serialize(e, t));
            else if ("POST" == this._tMode)
                s.loadXML(i, !0, this.serialize(e, t));
            else if ("REST" == this._tMode) {
                var a = this.getState(t), n = i.replace(/(\&|\?)editing\=true/, "");
        ...
        var a = e, n = "error" != s && "invalid" != s;
        if (n || this.set_invalid(e, s), this._uActions && this._uActions[s] && !this._uActions[s](i))
        ...
        if (window.JSON)
            try {
                var i = JSON.parse(t.xmlDoc.responseText), a = i.action || this.getState(s) || "updated", n = i.sid || s[0], r = i.tid || s[0];
                return e.afterUpdateCallback(n, r, a, i), void e.finalizeUpdate()
            } catch (d) {
            }
        if (t.getXMLTopNode("data"), !t.xmlDoc.responseXML)
            return this.obj && this.obj.callEvent && this.obj.callEvent("onSaveError", [s, t.xmlDoc]), this.cleanUpdate(s);
        var o = t.doXPath("//data/action");
        if (!o.length)
            return this.cleanUpdate(s);
        ...
        s = s.replace("editing=true&", ""), this.getUpdates(s, function () {
            ...
            if (s.length) {
                ...
                for (var i = 0; i < s.length; i++) {
                    var a = s[i].getAttribute("status"), n = s[i].getAttribute("id"), r = s[i].getAttribute("parent");
                    switch (a) {
                        case"inserted":
    ...
    if (this._drag_mode) {
        ...
        if (t.force_redraw || this._is_pos_changed(this._drag_pos, t)) {
            ...
            if (this._edit_id != this._drag_id && this._close_not_saved(), this._drag_pos = t, this._drag_pos.has_moved = !0, "create" == this._drag_mode) {
                ...
                var n = new Date(this._drag_start), r = new Date(i);
            ...
            if ("move" == this._drag_mode)
                s = this._min_date.valueOf() + 6e4 * (t.y * this.config.time_step + 24 * t.x * 60 - (scheduler._move_pos_shift || 0)), !t.custom && this._table_view && (s += 1e3 * this.date.time_part(o.start_date)), s = this._correct_shift(s), t._ignores && this.config.preserve_length && this._table_view ? (this.matrix && (d = this.matrix[this._mode]), d = d || {x_step: 1, x_unit: "day"}, i = 1 * s + this._get_fictional_event_length(s, this._drag_event._event_length, d)) : i = o.end_date.valueOf() - (o.start_date.valueOf() - s);
            else {
            ...
            if (scheduler.config.limit_drag_out && (+u < +scheduler._min_date || +i > +scheduler._max_date)) {
                var g = i - u;
                +u < +scheduler._min_date ? (u = new Date(scheduler._min_date), i = new Date(+u + g)) : (i = new Date(scheduler._max_date), u = new Date(+i - g));
                var c = new Date(i - 1)
            }
            if (!this._table_view && !scheduler.config.all_timed && (!scheduler._get_section_view() && t.x != this._get_event_sday({start_date: new Date(i), end_date: new Date(i)}) || new Date(i).getHours() >= this.config.last_hour)) {
                var g = i - u, h = this._min_date.valueOf() + 24 * t.x * 60 * 6e4;
    ...
    if (!e || 2 != e.button || !scheduler.config.touch) {
        if (this._drag_mode && this._drag_id) {
            ...
            var t = this._drag_id, s = this._drag_mode, i = this.getEvent(this._drag_id);
                ...
                if (this.callEvent("onBeforeEventChanged", [i, e, a, this._drag_event]))
                    if (this._drag_id = this._drag_mode = null, a && this.config.edit_on_create) {
                        if (this.unselect(), this._new_event = new Date, this._table_view || this.config.details_on_create || !this.config.select)
                            return scheduler.callEvent("onDragEnd", [t, s, e]), this.showLightbox(t);
                        this._drag_pos = !0, this._select_id = this._edit_id = t
                    } else
                        this._new_event || this.callEvent(a ? "onEventAdded" : "onEventChanged", [t, this.getEvent(t)]);
                else
                    a ? this.deleteEvent(i.id, !0) : (this._drag_event._dhx_changed = !1, this._clear_temporary_properties(i, this._drag_event), scheduler._lame_copy(i, this._drag_event), this.updateEvent(i.id))
            }
    ...
    var i = e, a = this._get_columns_num(t, s);
    this._process_ignores(t, a, "day", 1);
    for (var n = a - this._ignores_detected, r = 0; a > r; r++)
        this._ignores[r] ? (this._cols[r] = 0, n++) : this._cols[r] = Math.floor(i / (n - r)), i -= this._cols[r], this._colsS[r] = (this._cols[r - 1] || 0) + (this._colsS[r - 1] || (this._table_view ? 0 : this.xy.scale_width + 2));
    ...
    if (this.templates[this._mode + "_date"]) {
        ...
        t.className = "dhx_cal_data" + i, this._scales = {}, this._cols = [], this._colsS = {height: 0}, this._dy_shift = 0, this.set_sizes();
        var a, n, r = this._get_timeunit_start(), d = scheduler._get_view_end();
        if (a = n = this._table_view ? scheduler.date.week_start(r) : r, this._min_date = a, this._els.dhx_cal_date[0].innerHTML = this.templates[this._mode + "_date"](r, d, this._mode), this._max_date = d, scheduler._render_scales(e, t), this._table_view)
    ...
    if (n)
        for (var r = new Date(e), d = 0; t > d; d++)
            n(r) && (this._ignores_detected += 1, this._ignores[d] = !0, a && t++), r = scheduler.date.add(r, i, s), scheduler.date[s + "_start"] && (r = scheduler.date[s + "_start"](r))
    ...
    var a = scheduler.date.add(t, 1, "month"), n = new Date(s), r = scheduler._currentDate();
    ...
    var n = Math.ceil(Math.round((i.valueOf() - s.valueOf()) / 864e5) / 7), r = Math.floor(e.clientHeight / n) - this.xy.month_head_height;
    ...
    if (e && "object" == typeof e) {
        for (a = {}, i = [Array, Date, Number, String, Boolean], s = 0; s < i.length; s++)
            e instanceof i[s] && (a = s ? new i[s](e) : new i[s]);
        t.push(e, a);
        for (s in e)
            Object.prototype.hasOwnProperty.apply(e, [s]) && (a[s] = scheduler._lame_clone(e[s], t))
    }
    return a || e
    ...
    return null !== e.lightbox_id && void 0 !== e.lightbox_id
        ...
        for (var s = "var temp=date.match(/[a-zA-Z]+|[0-9]+/g);", i = e.match(/%[a-zA-Z]/g), a = 0; a < i.length; a++)
            switch (i[a]) {
                case"%j":
                case"%d":
                    s += "set[2]=temp[" + a + "]||1;";
    ...
    if (t) {
        ...
        if (s)
            return!0;
        var i = (e.end_date.valueOf() - e.start_date.valueOf()) / 36e5, a = 24 - (this.config.last_hour - this.config.first_hour);
        return i > a
    }
    return!1
    ...
    if (this.config.multi_day && !this._table_view) {
        for (var a = [], n = [], s = 0; s < e.length; s++)
            this._is_main_area_event(e[s]) ? a.push(e[s]) : n.push(e[s]);
    ...
    for (var t = this._rendered_location, s = this._lame_clone(e), i = e._sday; i < e._eday; i++) {
        ...
        var r = scheduler.getEvents(n, a.add(n, 1, "day")).length, d = this._get_event_bar_pos(s), o = d.x2 - d.x, l = document.createElement("div");
    ...
    var s = this.xy.bar_height, i = this._colsS.heights, a = this._colsS.heights = [0, 0, 0, 0, 0, 0, 0], n = this._els.dhx_cal_data[0];
    ...
    for (var a = 0; a < e.length; a++) {
        ...
        if (n._sday = this._get_event_sday(n), this._ignores[n._sday])
            e.splice(a, 1), a--;
        else {
    ...
    if (!t) {
        for (var a = 0; a < e.length; a++)
            e[a]._count = s[e[a]._sday].max_count;
        for (var a = 0; a < i.length; a++)
            i[a]._count = s[i[a]._sday].max_count
    }
    return e
    ...
    this._time_order(e);
    for (var s, i = [], a = [[], [], [], [], [], [], []], n = this._colsS.heights, r = this._cols.length, d = {}, o = 0; o < e.length; o++) {
        var l = e[o], h = l.id;
        ...
        if (l._sday = g % r, !this._ignores[l._sday] || !l._timed) {
            ...
            if (l._sorder && t || (l._sorder = v), l._sday + l._length <= r)
                s = null, i.push(l), m[v] = l, n[l._sweek] = m.length - 1, l._first_chunk = _.first_chunk, l._last_chunk = _.last_chunk;
            else {
    ...
    t = t || 0;
    var s = 60 * e.start_date.getHours() + e.start_date.getMinutes(), i = 60 * e.end_date.getHours() + e.end_date.getMinutes() || 60 * scheduler.config.last_hour, a = this._y_from_date(e.start_date), n = Math.max(t, (i - s) * this.config.hour_size_px / 60);
    ...
    if (!(e._sday < 0)) {
        ...
        if (i) {
            var a = this._calc_event_y(e, scheduler.xy.min_event_height), n = a.top, r = a.height, d = e._count || 1, o = e._sorder || 0, l = Math.floor((i.clientWidth - s) / d), h = o * l + 1;
    ...
    c && (_ = _ + " " + c);
    var u = e.color ? "background:" + e.color + ";" : "", g = e.textColor ? "color:" + e.textColor + ";" : "", f = '<div event_id="' + h + '" class="' + _ + '" style="position:absolute; top:' + s + "px; left:" + t + "px; width:" + (i - 4) + "px; height:" + a + "px;" + (n || "") + '"></div>';
    l.innerHTML = f;
    var v = l.cloneNode(!0).firstChild;
    if (!o && scheduler.renderEvent(v, e, i, a, r, d))
        return v;
    v = l.firstChild;
    ...
    var i = this.xy.bar_height, a = e._sorder;
    ...
    var r = a * i, d = this._colsS.heights[e._sweek] + (this._colsS.height ? this.xy.month_scale_height + 2 : 2) + r;
    return{x: t, x2: s, y: d}
}, scheduler.render_event_bar = function (e) {
    var t = this._rendered_location, s = this._get_event_bar_pos(e), i = s.y, a = s.x, n = s.x2, r = "";
    if (n) {
        ...
        g && (u += " " + g);
        var f = e.color ? "background:" + e.color + ";" : "", v = e.textColor ? "color:" + e.textColor + ";" : "", m = ["position:absolute", "top:" + i + "px", "left:" + a + "px", "width:" + (n - a - 15) + "px", v, f, e._text_style || ""].join(";"), p = '<div event_id="' + e.id + '" class="' + u + '" style="' + m + '">';
    ...
    if (e = e || this._load_url) {
        ...
        if (t = t || this._date, this._load_mode) {
            ...
            s = t;
            for (var a = !0; s < this._max_date; )
                s = this.date.add(s, 1, this._load_mode), this._loaded[i(t)] && a ? t = this.date.add(t, 1, this._load_mode) : a = !1;
            var n = s;
            do
                s = n, n = this.date.add(s, -1, this._load_mode);
            while (n > t && this._loaded[i(n)]);
            if (t >= s)
                return!1;
    ...
    e && "object" == typeof e && (this._obj.removeChild(e), this.config.show_loading = !0)
        ...
        var s = e.indexOf(":");
        if (-1 != s) {
            var i = e.substr(0, s).toLowerCase(), a = e.substr(s + 1).replace(/\\\,/g, ",").replace(/[\r\n]+$/, "");
        ...
        var a = i[0].substr(0, 4), n = parseInt(i[0].substr(4, 2), 10) - 1, r = i[0].substr(6, 2);
    ...
    var a = document.getElementById(i.id), n = a.nextSibling, r = {section: i, header: a, node: n, getValue: function (e) {
    ...
    return e.control = e.node.getElementsByTagName("select"), e
            ...
            var t = e.time_format, s = scheduler.config, i = this.date.date_part(scheduler._currentDate()), a = 1440, n = 0;
            ...
            var n, r, d = scheduler.config, o = e.getElementsByTagName("select"), l = i._time_format_order;
            if (d.full_day) {
                if (!e._full_day) {
            ...
            var i = e.getElementsByTagName("select"), a = s._time_format_order;
    ...
    for (var t = this.config.lightbox.sections, s = 0; s < t.length; s++) {
        ...
        i = i ? i.nextSibling : i;
        var a = this.form_blocks[t[s].type], n = a.get_value.call(this, i, e, t[s]);
            ...
            switch (t.className) {
                ...
                default:
                    if (t.getAttribute("dhx_button"))
                        ...
                        var i, a, n;
                        -1 != t.className.indexOf("dhx_custom_button") && (-1 != t.className.indexOf("dhx_custom_button_") ? (i = t.parentNode.getAttribute("index"), n = t.parentNode.parentNode) : (i = t.getAttribute("index"), n = t.parentNode, t = t.firstChild)), i && (a = scheduler.form_blocks[scheduler.config.lightbox.sections[i].type], a.button_click(i, t, n, n.nextSibling))
                    }
                }
    ...
    function n(e) {
        var t = scheduler.getState().drag_mode, s = scheduler.matrix ? scheduler.matrix[scheduler._mode] : !1, i = scheduler.render_view_data;
        "create" == t && s && (scheduler.render_view_data = function () {
            for (var e = scheduler.getState().drag_id, t = scheduler.getEvent(e), i = s.y_property, a = scheduler.getEvents(t.start_date, t.end_date), n = 0; n < a.length; n++)
                a[n][i] != t[i] && (a.splice(n, 1), n--);
    ...
    i(document.body, e[0], function (e) {
        if (!s(e)) {
            if (h)
                return n(t(e)), scheduler._update_global_tip(), e.preventDefault && e.preventDefault(), e.cancelBubble = !0, !1;
            if (o && u && a(o, t(e), 0), o = t(e), c)
                return o ? void((d.target != o.target || Math.abs(d.pageX - o.pageX) > 5 || Math.abs(d.pageY - o.pageY) > 5) && (_ = !0, clearTimeout(l))) : void(_ = !0)
        }
        ...
        if (!s(e)) {
            var i;
            if (h = _ = !1, c = !0, scheduler._temp_touch_block = !0, i = o = t(e), !i)
                return void(_ = !0);
            var a = new Date;
            if (!_ && !h && 250 > a - g)
        ...
        a && (a.padding = 14), scheduler.templates.event_bar_date = function (e) {
            ...
            if (scheduler.templates.month_day = function (s) {
                ...
                for (var s = scheduler._els.dhx_cal_navline[0].getElementsByTagName("div"), i = null, a = 211, n = 0; n < s.length; n++) {
                    var r = s[n], d = r.getAttribute("name");
                    if (d)
    ...
    var t = scheduler.setCurrentView, s = scheduler.updateView, i = null, a = null, n = function (t, n) {
        var r = this;
